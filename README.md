...
1. 새 시스템 클래스 생성 (예: `DashSystem.cs`)
2. Player의 Awake/Start에서 초기화
3. Update/FixedUpdate에서 처리
4. Player 인스펙터에서 할당

### 기존 기능 수정

각 시스템이 독립적이므로 해당 클래스만 수정:

```csharp
// 예: 와이어 힘 조정
// WireSystem.cs의 pullForce 변경 → 독립적 적용
```
...
버그 리포트 및 기능 제안은 Issues 탭에서 부탁합니다.
